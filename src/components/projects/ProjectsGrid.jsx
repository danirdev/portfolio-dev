    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {PROJECTS.map((project, idx) => (
        <GridCard key={project.id} title={project.title} label={`MOD_${idx + 1}`}>
          <p className="font-mono text-xs text-gray-600 mb-4 h-12">
            {project.desc}
          </p>
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <span className={`text-[10px] font-bold px-1 ${project.status === 'ONLINE' ? 'bg-green-100 text-green-700' :
            ...
            <a
              ...
              className={`text-xs font-bold hover:underline flex items-center gap-1 ${project.status === 'OFFLINE' ? 'opacity-50 pointer-events-none' : ''
                }`}
            >
